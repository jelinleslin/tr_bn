...
Factor_Tree::Factor_Tree(std::vector<int> parents_inner, std::vector<int> parents_leave, std::vector<std::vector<int>> vec_variables_leave, std::vector<int> num_categories) {
  ...
  for(int i = 0; i < vec_variables_leave.size(); ++i){
  ...
  for(int i = 0; i < parents_inner.size(); ++i){
  ...
  for(int i = 0; i < parents_leave.size(); ++i){
...
//  td = clock();
//  t1 += double(tb-ta)/  CLOCKS_PER_SEC;
//  t2 += double(tc-tb)/  CLOCKS_PER_SEC;
//  t3 += double(td-tc)/  CLOCKS_PER_SEC;
...
double Factor_Tree::ll_parallel(int* data, int ncols, int nrows){
    Factor_Tree ft = *this;
    std::vector<double> ll_v(nrows, 0.0);
    double ll = 0;

    //TODO: Revisar variables compartidas
//    #pragma omp parallel for firstprivate(ft) shared(ll_v)
    for (auto i=0;i<nrows;++i){
        std::vector<int> q_values;
        std::vector<int> query_i;
        double prob_q;
        // Obtain the positions of the observed values  in the row 
        for (auto j=0;j<ncols;++j){
            auto value = data[pos_datav(i, j, nrows)];
            if(value>=0){
        ...
        //Compute probability of joint query 
        ft.set_evidence(query_i, q_values);
        ft.sum_compute_tree();
        prob_q = ft.root->factor->prob.at(0);
        ft.retract_evidence();
        //Compute probability of joint query 
        if (prob_q!=0){ll_v[i] = log(prob_q);}
        else{
            std::cout << "Warning: ll_parallel. Case " << i << " has probability 0. Min double is used to approximate the log-likelihood function\n";
            ll_v[i] += log(std::numeric_limits<double>::denorm_min());
        }
//        std::cout << "inst t1: " << t1 << ", t2: " << t2 << ", t3: " << t3 << ", t4: " << t4 << "\n";
    }
//    std::cout << "t1: " << t1 << ", t2: " << t2 << ", t3: " << t3 << ", t4: " << t4 << "\n";
    for (auto lli=ll_v.begin();lli!=ll_v.end();++lli){
        ll += *lli;
//        std::cout << "lli: " << *lli << "\n";
    }
//    std::cout << "ll: " << ll << "\n";
...
double Factor_Tree::cll_parallel(int* data, int ncols, int nrows, std::vector<int> query, std::vector<int> weights){
    Factor_Tree ft = *this;
    // Complete each row with missing values
    double cll = 0;
    std::vector<bool> query_flag(ncols, false);
    ...
    for (auto i=0;i<nrows;++i){
        ...
        prob_q = ft.root->factor->prob.at(0);
        ft.retract_evidence();
        ft.set_evidence(ev_i, e_values);
        ft.sum_compute_tree();
        prob_e = ft.root->factor->prob.at(0);
        ft.retract_evidence();
        ...
        if (prob_e == 0){
            std::cout << "prob of evidence = 0 \n";
            std::cout << "ev: ";
            print_vector(ev_i);
            std::cout << "\n values: ";
            print_vector(e_values);
            std::cout << "\n";
            //throw std::runtime_error( "prob of evidence = 0 \n");
        } else{
            if (prob_q>0){
                cll += log(prob_q/prob_e) * weights.at(i);
            }
        }
...
double Factor_Tree::cll(int* data, int ncols, int nrows, std::vector<int> query, std::vector<int> weights){
    Factor_Tree ft = *this;
    // Complete each row with missing values
    double cll = 0;
    std::vector<bool> query_flag(ncols, false);
...
std::vector<std::vector<double>> Factor_Tree::pred_data(int* data, int ncols, int nrows, std::vector<int> qvalues, std::vector<int> query, std::vector<int> evidence){
    Factor_Tree ft = *this;
    // Complete each row with missing values
    std::vector<std::vector<double>> probs(query.size(),std::vector<double>(nrows, 0));
    //#pragma omp parallel for firstprivate(ft)
    for (auto i=0;i<nrows;++i){
        ...
        ft.sum_compute_tree();
        double prob_e = ft.root->factor->prob.at(0);
        ft.retract_evidence();
        if (prob_e == 0){
            throw std::runtime_error( "prob of evidence = 0 \n");
        }
...
void Factor_Tree::mpe_data_oneproc(int* data_complete, const int* data_missing, int ncols, int nrows, std::vector<int> rows, std::vector<double> &prob_mpe){
    Factor_Tree ft = *this;
    // Complete each row with missing values
    std::vector<int> values;
...
/*Predict marginals for data using distribute evidence*/
std::vector<std::vector<std::vector<double>>> Factor_Tree::pred_data_distribute(int* data, int ncols, int nrows, std::vector<int> query, std::vector<int> evidence){
    Factor_Tree ft = *this;
    // Complete each row with missing values
    std::vector<std::vector<std::vector<double>>> probs(nrows,std::vector<std::vector<double>>(query.size()));
//    #pragma omp parallel for firstprivate(ft) shared(probs)
    for (auto i=0;i<nrows;++i){
        std::vector<int> evidence1;
        std::vector<int> values;
        // Obtain the positions of the observed values  in the row 
        ...
        double prob_e = ft.root->factor->prob.at(0);
        if (prob_e == 0){
            std::cout << "Warning: pred_data_distribute. Prob of evidence is 0 in row " << i << "\n";
        }
        else{
            ft.distribute_evidence();
            ft.distribute_leaves(evidence, values, ncols);
            ...
            for (auto j=0;j<query.size();++j){
                // Get joint factor of node j
                auto fact_j = ft.leave_nodes[j]->joint_factor;
                auto vars = fact_j->variables;
                
                // Sum over all the nodes that are not j
                for (auto xm=vars.begin(); xm!=vars.end();++xm){
                    if (*xm!=j){fact_j = sum_factor(*fact_j, *xm);}
                }
                
                // Return the probability of all the values
                auto probs_j = fact_j->prob;
                //for (auto x=probs_j.begin();x!=probs_j.end();++x){ *x = *x/prob_e;}
                probs[i][j] = probs_j;
            }
        }
        ft.retract_evidence();
    ...
    /*Get sum logP(xi|X_rest), where X_rest all the variables minus x_i*/

double Factor_Tree::pseudo_likelihood(int* data, int ncols, int nrows, int xi, std::vector<int> ch_xi){
    double pll = 0;
    unsigned ncat_xi = this->leave_nodes[0]->factor->num_categories[xi];
    /*Get factor idx that contain xi in their domain*/
    std::vector<int> x_change = ch_xi; 
    x_change.push_back(xi);
    // For each row
    for (auto i = 0; i < nrows; ++i){
        double pxi=1;
        unsigned nxi=0, classi=0;
        std::vector<double> norm_v(ncat_xi,1);
        // For each factor
        for (auto nxj = x_change.begin(); nxj!=x_change.end(); ++nxj){
            auto xj = *nxj;
            auto factor = this->leave_nodes[xj]->factor;
            auto variables = factor->variables; 
            //get values from data
            std::vector<int> values(variables.size());
            unsigned ni = 0;
            for (auto vi = variables.begin(); vi!=variables.end(); ++vi){
                values[ni] = data[i + (*vi)*nrows];
                if (values[ni] < 0){
                    throw std::runtime_error("Missing data not supported.");    
                }
                if (*vi == xi) {
                    nxi = ni;
                    classi = data[i + (*vi)*nrows];
                }
                ni++;
            }
            //get position from values
            auto pos = position_in_factor(values, factor->variables, factor->num_categories);
            //Update pxi
            pxi *= factor->prob[pos];
            norm_v[classi] *= factor->prob[pos];
            //Update norm
            for (auto ci = 0; ci<ncat_xi; ++ci){
                if (ci != classi){
                    values[nxi] = ci;
                    auto pos = position_in_factor(values, factor->variables, factor->num_categories);
                    norm_v[ci] *= factor->prob[pos];
                }
            }
        }
        double norm = 0;
        for (auto normi = norm_v.begin(); normi!= norm_v.end(); ++normi){
            norm += *normi;
        }
        pll += log(pxi) - log(norm);
    }
    return pll;
}


double pseudo_likelihood2(int* data, int ncols, int nrows, int xi, std::vector<int> ch_xi, std::vector<std::shared_ptr<Factor>> factors){
    double pll=0;
    unsigned ncat_xi = factors[0]->num_categories[xi];
    /*Get factor idx that contain xi in their domain*/
    std::vector<int> x_change = ch_xi; 
    x_change.push_back(xi);
    std::vector<double> pll_v(nrows);
    // For each row
    #pragma omp parallel for firstprivate(ncat_xi, x_change, ncols, xi, nrows) shared(data,pll_v, factors)
...
void remove(std::vector<int> &v, int i){
    auto var_iterator=std::find(v.begin(), v.end(), i);
    auto index = std::distance(v.begin(), var_iterator);
    v.erase(var_iterator);     
}


//Compute pseudo_cll for a local change. change_type = 0 (addition), 1 (removal), 2 (reversal)
std::vector<double> Factor_Tree::pseudo_cll_score(int change_type,int xout, int xin, int* data, int ncols, int nrows, std::vector<std::vector<int>> &children, std::vector<int> &class_vars, std::vector<double> &pseudocll_old){
    
    // Create a vector with the factors of the FT 
    std::vector<std::shared_ptr<Factor>> factors;
    
    for(auto ni=this->leave_nodes.begin(); ni!=this->leave_nodes.end(); ++ni){
        factors.push_back((*ni)->factor);
    }
    
    std::vector<double> pseudo_cll = pseudocll_old;
    
    if (change_type == 0){
        std::vector<std::vector<int>>  new_children = children;
        std::vector<int> variables = factors[xin]->variables;
        variables.push_back(xout);
        //xin is a child of xou
        new_children[xout].push_back(xin);
        // Create a new vector with the parameters after the change
        std::shared_ptr<Factor> new_f = std::make_shared<Factor>(variables, factors[xin]->num_categories);
        new_f->learn_params(data, nrows, ncols, 0);
        factors[xin] = new_f;
        //Compute new pseudo_cll
        for (auto nj = variables.begin(); nj!=variables.end(); ++nj){
            auto xj = *nj;
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
        }
    } else if(change_type == 1){
        std::vector<std::vector<int>>  new_children = children;
        std::vector<int> variables_backup = factors[xin]->variables;
        std::vector<int> variables = factors[xin]->variables;
        
        remove(variables,xout);
        remove(new_children[xout],xin);
                
        std::shared_ptr<Factor> new_f = std::make_shared<Factor>(variables, factors[xin]->num_categories);
        new_f->learn_params(data, nrows, ncols, 0);
        factors[xin] = new_f;
        for (auto nj = variables_backup.begin(); nj!=variables_backup.end(); ++nj){
            auto xj = *nj;
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
        ...
        std::vector<std::vector<int>>  new_children = children;
        std::vector<int> variables_comp = factors[xin]->variables;
        std::vector<int> variables = factors[xin]->variables;
        ...
        std::vector<int> variables_add = factors[xout]->variables;
        variables_add.push_back(xin);
        new_children[xin].push_back(xout);
        
        new_f = std::make_shared<Factor>(variables_add, factors[xout]->num_categories);
        new_f->learn_params(data, nrows, ncols, 0);
        factors[xout] = new_f;
        
        for (auto nj = variables_add.begin(); nj!=variables_add.end(); ++nj){
            auto xj = *nj;
            if(std::find(variables_comp.begin(), variables_comp.end(), xj) == variables_comp.end()){
                variables_comp.push_back(xj);
            }
        }
        for (auto nj = variables_comp.begin(); nj!=variables_comp.end(); ++nj){
            auto xj = *nj;
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
...
std::vector<double> pseudo_cll_score(const Factor_Tree &ft, int change_type,int xout, int xin, int* data, int ncols, int nrows, std::vector<std::vector<int>> &children, std::vector<int> &class_vars, std::vector<double> &pseudocll_old){
    
    // Create a vector with the factors of the FT 
    std::vector<std::shared_ptr<Factor>> factors;
    
    
    for(auto ni=ft.leave_nodes.begin(); ni!=ft.leave_nodes.end(); ++ni){
        factors.push_back((*ni)->factor);
    }
    ...
    if (change_type == 0){
        ...
        factors[xin] = new_f;
        //Compute new pseudo_cll
        for (auto nj = 0; nj<variables.size(); ++nj){
            auto xj = variables[nj];
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
        }
        ...
        new_f->learn_params(data, nrows, ncols, 0);
        factors[xin] = new_f;
        for (auto nj = 0; nj<variables_backup.size(); ++nj){
            auto xj = variables_backup[nj];
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
        }    
        ...
        for (auto nj = 0; nj<variables_comp.size(); ++nj){
            auto xj = variables_comp[nj];
            if(std::find(class_vars.begin(), class_vars.end(), xj) != class_vars.end()){
                pseudo_cll[xj] = pseudo_likelihood2(data, ncols, nrows, xj, new_children[xj], factors);
            }
        }
