...
class Factor_Tree {
  public:
    ...
    std::vector<std::vector<double>> pred_data(int* data, int ncols, int nrows, std::vector<int> qvalues, std::vector<int> query, std::vector<int> evidence);
    /*Predicts marginals for each row in data_complete */
    ...
    std::vector<std::vector<std::vector<double>>> pred_data_distribute(int* data, int ncols, int nrows, std::vector<int> query, std::vector<int> evidence);
    
    /*Get sum logP(xi|X_rest), where X_rest all the variables minus x_i*/
    double pseudo_likelihood(int* data, int ncols, int nrows, int xi, std::vector<int> ch_xi);

    std::vector<double> pseudo_cll_score(int change_type,int xout, int xin, int* data, int ncols, int nrows, std::vector<std::vector<int>> &children, std::vector<int> &class_vars, std::vector<double> &pseudocll_old);
